  ...
  case $state in
    (args)
      case $line[1] in
        ...
        (openclaw-weixin) _openclaw_openclaw_weixin ;;
...
_openclaw_openclaw_weixin_uninstall() {
  _arguments -C \
    
}

_openclaw_openclaw_weixin_logs_upload() {
  _arguments -C \
    "--url[Remote URL to POST the log file to (overrides config)]" \
    "--file[Log file to upload: full filename or 8-digit date YYYYMMDD (default: today)]"
}

_openclaw_openclaw_weixin() {
  local -a commands
  local -a options
  
  _arguments -C \
     \
    "1: :_values 'command' 'uninstall[Uninstall the Weixin plugin (cleans up channel config automatically)]' 'logs-upload[Upload a Weixin log file to a remote URL via HTTP POST]'" \
    "*::arg:->args"

  case $state in
    (args)
      case $line[1] in
        (uninstall) _openclaw_openclaw_weixin_uninstall ;;
        (logs-upload) _openclaw_openclaw_weixin_logs_upload ;;
      esac
      ;;
  esac
}
